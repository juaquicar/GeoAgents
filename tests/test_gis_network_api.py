"""
Tests de API para spatial.network_service_area usando mocks (sin BD real).
Cubre el flujo completo de creación/ejecución de runs y contratos de respuesta.

Ejecutar:
    python manage.py test tests.test_gis_network_api
"""
...
class NetworkServiceAreaApiTests(APITestCase):
    def setUp(self):
        ...
        # Agente sin gis_db_connections — los tests usan mocks de invoke_tool
    ...
    def _direct_tool_payload(self, args=None, goal="calcular area de servicio de red"):
        ...
        return {
            ...
            "input_json": {
                "goal": goal,
                "tool_call": {"name": "spatial.network_service_area", "args": args},
            },
        }
    ...
    def test_create_and_execute_direct_network_service_area_run_via_api(self, mock_invoke_tool):
        mock_invoke_tool.return_value = (
            _tool_ok({
                ...
                "origin_snap_m": 44.21,
                "metric": "cost",
                "limits": {"max_cost": 300.0, "max_distance_m": None},
                ...
                "coverage_summary": {
                    "total_network_nodes": 5, "total_network_segments": 4,
                    "total_network_length_m": 826.86, "total_network_cost": 826.86,
                    "node_coverage_ratio": 0.6, "segment_coverage_ratio": 0.5,
                    "length_coverage_ratio": 0.5705, "cost_coverage_ratio": 0.5705,
                },
                "reachable_nodes": [
                ...
                "coverage_bbox": {"west": -6.0570, "south": 37.3235, "east": -6.0540, "north": 37.3270},
                "service_segments": [
                    {
                        "segment_id": 3, "segment_name": "Tramo 3", "segment_type": None,
                        "length_m": 213.04, "edge_cost": 213.04,
                        "geom_geojson": '{"type":"LineString","coordinates":[[-6.057,37.3235],[-6.0555,37.325]]}',
                    },
                    {
                        "segment_id": 4, "segment_name": "Tramo 4", "segment_type": None,
                        "length_m": 258.73, "edge_cost": 258.73,
        ...
        self.assertEqual(execute_response.data["output_json"]["tool"], "spatial.network_service_area")
        self.assertEqual(execute_response.data["output_json"]["data"]["reachable_segment_ids"], [3, 4])
        self.assertEqual(execute_response.data["output_json"]["data"]["reachable_segment_count"], 2)
        self.assertEqual(execute_response.data["output_json"]["data"]["reachable_node_count"], 3)
        self.assertEqual(
            execute_response.data["output_json"]["data"]["coverage_summary"]["segment_coverage_ratio"], 0.5
        )
        self.assertEqual(execute_response.data["output_json"]["data"]["limits"]["max_cost"], 300.0)
        ...
        self.assertIn("network_service_area", execute_response.data["run_memory"]["analysis_types"])
        self.assertEqual(execute_response.data["episode"]["outcome_status"], "succeeded")
    ...
    def test_direct_network_service_area_snap_distance_exceeded_via_api(self, mock_invoke_tool):
        mock_invoke_tool.return_value = (
            _tool_ok({
                ...
                "coverage_summary": {
                    "total_network_nodes": 5, "total_network_segments": 4,
                    "total_network_length_m": 826.86, "total_network_cost": 826.86,
                    "node_coverage_ratio": 0.0, "segment_coverage_ratio": 0.0,
                    "length_coverage_ratio": 0.0, "cost_coverage_ratio": 0.0,
                },
                "reachable_nodes": [],
                "coverage_bbox": None,
                "service_segments": [],
        ...
        execute_response = self._execute_run_via_api(create_response.data["id"])

        self.assertEqual(execute_response.data["status"], "succeeded")
        self.assertFalse(execute_response.data["output_json"]["data"]["reachable"])
        self.assertEqual(execute_response.data["output_json"]["data"]["reason"], "snap_distance_exceeded")
        self.assertEqual(execute_response.data["output_json"]["data"]["reachable_segment_ids"], [])
        self.assertIsNone(execute_response.data["output_json"]["data"]["coverage_bbox"])
        self.assertIsNotNone(execute_response.data["run_memory"])
        self.assertEqual(execute_response.data["run_memory"]["domain"], "network")
    ...
    def test_direct_network_service_area_default_limit_and_trace_visibility_via_api(self, mock_invoke_tool):
        mock_invoke_tool.return_value = (
            _tool_ok({
                ...
                "origin_snap_m": 44.21,
                "metric": "cost",
                "limits": {"max_cost": 1500.0, "max_distance_m": None},
                ...
                "coverage_summary": {
                    "total_network_nodes": 5, "total_network_segments": 4,
                    "total_network_length_m": 826.86, "total_network_cost": 826.86,
                    "node_coverage_ratio": 1.0, "segment_coverage_ratio": 1.0,
                    "length_coverage_ratio": 1.0, "cost_coverage_ratio": 1.0,
                },
                "reachable_nodes": [],
                "coverage_bbox": {"west": -6.0600, "south": 37.3200, "east": -6.0500, "north": 37.3300},
                "service_segments": [],
            }),
            11,
        )
        ...
        self.assertEqual(execute_response.data["status"], "succeeded")
        self.assertEqual(execute_response.data["output_json"]["data"]["limits"]["max_cost"], 1500.0)
        self.assertEqual(execute_response.data["output_json"]["data"]["reachable_segment_count"], 4)

        detail_response = self.client.get(reverse("runs-detail", kwargs={"pk": run_id}))
        self.assertEqual(detail_response.status_code, 200)
        self.assertEqual(detail_response.data["output_json"]["tool"], "spatial.network_service_area")

        steps_response = self.client.get(reverse("runs-steps", kwargs={"pk": run_id}))
        ...
        trace_response = self.client.get(reverse("runs-trace", kwargs={"pk": run_id}))
        ...
        self.assertEqual(trace_response.data["trace"]["verification_summary"]["counts"]["verified"], 0)
    ...
    def test_full_planner_flow_network_service_area_returns_final_text(
        ...
        create_response = self._create_run_via_api(
            {
                "agent": self.agent.id,
                "input_json": {
                    "goal": "analiza el área de cobertura de red desde el nodo principal",
                    "map_context": {
                        "bbox": {"west": -6.06, "south": 37.32, "east": -6.05, "north": 37.33},
                        "zoom": 16,
                    },
        ...
        self.assertEqual(execute_response.data["status"], "succeeded")
        self.assertEqual(
            execute_response.data["final_text"],
            "La red cubre 2 tramos desde el origen, alcanzando el 57% de la longitud total.",
        )
        self.assertEqual(len(execute_response.data["executed_outputs"]), 1)
        step_out = execute_response.data["executed_outputs"][0]
        self.assertEqual(step_out["name"], "spatial.network_service_area")
        self.assertTrue(step_out["ok"])
        self.assertEqual(step_out["verification"]["status"], "verified")
        ...
        self.assertEqual(trace_response.status_code, 200)
        self.assertEqual(
            trace_response.data["trace"]["goal"],
            "analiza el área de cobertura de red desde el nodo principal",
        )
        self.assertEqual(
    ...
    def test_execute_direct_network_service_area_is_forbidden_for_other_user_run(self):
        foreign_run = Run.objects.create(
            ...
            input_json={
                ...
                "tool_call": {
                    "name": "spatial.network_service_area",
                    "args": {"layer": "demo_lines", "origin_point": {"lon": -6.055, "lat": 37.325}, "max_cost": 300},
                },
            },
        )
